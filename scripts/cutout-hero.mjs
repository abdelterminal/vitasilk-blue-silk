...
const SRC = process.argv[2] ?? "C:/Users/Brandshift 01/Downloads/vitasilk coffee img/bottle-source.png";
const OUT = "assets/images";

const DENOISE = 1.5; // blur applied before the Sobel, to kill per-row render banding
const EDGE = 3; // Sobel magnitude counted as an outline
const CLOSE = 3; // dilation radius used to seal pinholes in the ring (undone in 6b)
const ERODE = 0.5; // alpha midpoint after blur (CLOSE is undone morphologically, see 6b)
const SOFT = 1.2; // edge feather radius
...
// 6. Keep the largest blob — drops the contact shadow and any stray speckle.
...
for (let p = 0; p < W * H; p++) blob[p] = label[p] === best ? 1 : 0;
...
const tight = erodeMask(blob, CLOSE);
